...
import { Text, View, TouchableOpacity, StyleSheet, FlatList, ScrollView, TextInput } from 'react-native';
                  ...
                  <View style={{ marginTop: 22, flex:1, flexDirection: 'column'}}>
                      ...
                      <ScrollView>
                         <FlatList
                              ...
                              renderItem={({item}) => 
                              (item.isAdded) ? <View style={Styles.formGroup}>
                                          <Text style={Styles.currency}>₹ </Text>
                                            <TextInput                                      
                                            onEndEditing={(e: any) => this.setComponentValue(item.key, e.nativeEvent.text)}
                                            placeholder={'0'}
                                            style={Styles.input}
                      ...
                      <View style={Styles.formGroup}>
                        <Text style={Styles.currency}>₹ </Text>
                        <Text style={{ paddingTop: 10, paddingBottom: 10, fontSize: 24, fontWeight: 'bold' }}>{this.props.components.totalAmount}</Text>
                        <Text style={Styles.label}>Total</Text>
                      </View>
                      <TouchableOpacity style={Styles.next} onPress={() => this.props.navigation.navigate('addMembers')}>
...
export default connect(mapStateToProps, mapDispatchToProps)(SetComponentValue);
