...
import { ScrollView, Text, TextInput, View, Button, FlatList, StyleSheet, Image, Modal, TouchableOpacity } from 'react-native';
...
class AddMembers extends Component {
  ...
  saveNewMember() {
    this.props.addMember({ name: this.state.name, amountPaid: this.state.amountPaid });
  ...
  render() {
    return (
                ...
                <Text>Contribution to be paid -> {this.props.components.totalAmount}</Text>
      <ScrollView>
        <FlatList
            ...
            />
                ...
                <View>
               <Button title={'Remove'} onPress={() => this.props.removeMember(item.key)} />
                </View>
                </View>}
               />
           <Button title={'Add New Member'} onPress={() => this.setModalVisible(true)} />
          ...
          <Ionicons style={styles.arrow} name={'ios-arrow-forward'} size={25} color={'#000000'} /> 
        </TouchableOpacity>
    </View>
    );
  } 
